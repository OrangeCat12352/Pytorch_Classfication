...
data_transform = transforms.Compose([
        ...
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.444388, 0.53172976, 0.30675733], std= [0.16900708, 0.15405944, 0.15587977])
    ])
...
def test():
    with torch.no_grad():
        ...
        for iteration, (images, labels) in enumerate(test_loader):
            ...
            print("===> Epoch[{}] =====>Mean_val_Acc:{:.4f},0_Acc:{:.4f},1_Acc:{:.4f},2_Acc:{:.4f},3_Acc:{:.4f}".format(
                iteration, test_acc / total, test_acc0 / total0, test_acc1 / total1, test_acc2 / total2,
                           test_acc3 / total3))
    ...
    return test_acc / total, np.array(tmp_prediction), np.array(total_label), np.array(tmp_scores), np.array(
        total_features)
...
if __name__ == '__main__':
    ...
    model = swinv2_tiny_window8_256(pretrained=False)
    ...
    for n in range(len(model_names)):
        ...
        if 1:
            ...
            # last_log = 'checkpoint/{}/seq_last.pth'.format(model_names[n])
            ...
            if 1:
                ...
                confusion_matrix_acc = confusion_matrix_test.astype('float') / confusion_matrix_test.sum(axis=1)[:,
                                                                               np.newaxis]
